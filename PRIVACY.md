...
ClaroIA can optionally call a local loopback Privacy Filter sidecar before generation. Cloud calls are blocked by default when the filter is enabled but unavailable. In mask mode, only masked text is sent to the LLM and saved/exported. The app does not store a reversible re-identification map. This reduces exposure but is not legal anonymization or a compliance guarantee.
